...
export default async function HalkYatirimIndicators({
  ticker,
}: {
  ticker: string
}) {
  ...
  return (
    <>
      <p
        className="text-xl font-semibold text-neutral-500 dark:text-neutral-500"
        style={{ padding: "24px" }}
      >
      ...
      <div className="grid grid-cols-3 grid-rows-2 gap-4">
        {Object.keys(newsData.indicators).map((item: any, index: number) => (
          <Card className="min-h-[15rem]" key={index}>
            <p
